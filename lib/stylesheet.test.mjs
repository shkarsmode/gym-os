...
test("a focused set field clears every sticky layer above it", () => {
    /**
     * When the on-screen keyboard opens, the browser scrolls the focused input into view
     * measured against the TOP OF THE VIEWPORT — it knows nothing about position:sticky.
     * Three layers are parked up there on the workout screen (topbar, action bar,
     * «Минулого разу» strip, ~199px together on a 390px phone), so the field being typed
     * into landed underneath all of them. Measured before the fix: scrollIntoView left it
     * at top 0, i.e. 199px out of sight.
     *
     * scroll-margin-top is what tells the browser's own scrolling where the readable area
     * starts, so all three heights have to appear in it. They are published as custom
     * properties by updateTopbarOffset() precisely so this can be written down.
     */
    const source = stripComments(fs.readFileSync(cssPath, "utf8"));
    ...
    assert.ok(margin, "the set field has no scroll-margin-top:\n" + block);
    for (const variable of ["--topbar-h", "--wab-h", "--lr-h"]) {
        assert.ok(
            margin.includes(variable),
            `scroll-margin-top must account for ${variable} — every sticky layer counts, `
            + `and missing one leaves the field that much further under the header:\n` + margin
        );
    }
});
