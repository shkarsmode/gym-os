...
test("a set field does not reserve room for sticky chrome it cannot see", () => {
    /**
     * This guard replaces its own opposite, so the reasoning is worth keeping.
     *
     * The tempting rule is: the topbar, the action bar and the «Минулого разу» strip are
     * sticky, so `scroll-margin-top` should reserve all three (~200px) and the browser's
     * focus scroll will stop below them. That holds where opening the keyboard only
     * SHRINKS the visual viewport — the chrome stays at the top of what you can see.
     *
     * iOS does not do that. It scrolls the page past its own top edge to lift the input
     * above the keyboard, and sticky elements go with the layout viewport — so the header
     * is off screen entirely. Reserving 200px for chrome that is not there sent the page
     * travelling that much further than it needed to and made the headers judder, which
     * is exactly what shipping it produced.
     *
     * The opening scroll therefore belongs to the browser. The case that genuinely needs
     * correcting is the field left under the header once the keyboard has GONE, and that
     * is done in JS, measuring what is actually on screen at that moment.
     */
    const source = stripComments(fs.readFileSync(cssPath, "utf8"));
    const block = (source.match(/\.set-field,[^{]*\{[^}]*\}/) || [])[0];
    assert.ok(block, "no .set-field scroll-margin rule found");
    const margin = (block.match(/scroll-margin-top\s*:[^;]+;/) || [])[0] || "";
    for (const variable of ["--topbar-h", "--wab-h", "--lr-h"]) {
        assert.equal(
            margin.includes(variable),
            false,
            `scroll-margin-top must NOT reserve ${variable}: on iOS that chrome has already `
            + `scrolled out of view when the keyboard is up, so the reservation only `
            + `overscrolls the page. Fix the after-close case in revealEditedField instead.`
            + `\n` + margin
        );
    }
});
