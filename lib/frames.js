...
// Each 5-tier band speaks its own SHAPE language (silhouette, not just colour), and the
// generic effect axes still only ever switch ON as the tier rises. Bands:
//   metal (1-5)    clean bevelled ring
//   crystal (6-10) diamond gem anchors on the corners
//   neon (11-15)   outer neon tube outline + side energy nodes
//   flame (16-20)  flame shards rising from the top corners + sparkles
//   holo (21-24)   asymmetric prism arcs + orbit ring + hue-shifting spin
//   apex (25)      unique endgame: crown, orbiting fragments, double aura, counter-arcs
const SHAPE_BANDS = ["metal", "crystal", "neon", "flame", "holo"];
...
export const FRAME_TIERS = TIER_ROWS.map(([name, colors, glow, width, kind], index) => ({
    ...
    glow,
    width,
    shape: index === TIER_ROWS.length - 1 ? "apex" : SHAPE_BANDS[Math.min(4, Math.floor(index / 5))],
    conic: kind === "conic",
    anim: kind === "conic",
    sheen: kind !== "matte",
    ornament: index >= 10,
