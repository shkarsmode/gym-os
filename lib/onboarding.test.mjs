...
import { birthDateBounds } from "./onboarding.js";

test("the birthday field declares bounds the picker can grey out", () => {
    const bounds = birthDateBounds(new Date("2026-08-21T12:00:00Z"));
    assert.equal(bounds.max, "2016-08-21");
    assert.equal(bounds.min, "1926-08-21");
    // The same bounds the validator enforces, so the picker cannot offer a date that
    // «Далі» will then refuse.
    assert.equal(validBirthDate(bounds.max, new Date("2026-08-21T12:00:00Z")), true);
    assert.equal(validBirthDate(bounds.min, new Date("2026-08-21T12:00:00Z")), true);
});

test("a picked-but-impossible date says why, not «pick a date»", () => {
    // What actually happened: the calendar could not reach a real birth year, somebody
    // entered 2025, and the app answered a visibly-filled field with "choose a date".
    assert.match(stepBlocker("birth", { birthDate: "2025-05-01" }), /вкажи свій рік/);
    assert.match(stepBlocker("birth", { birthDate: "1890-01-01" }), /[Пп]еревір рік/);
    assert.equal(stepBlocker("birth", { birthDate: "" }), "Обери дату народження");
    assert.equal(stepBlocker("birth", { birthDate: "не дата" }), "Обери дату народження");
});
