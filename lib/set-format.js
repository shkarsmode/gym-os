...
/**
 * What to call the weight field for a given exercise.
 *
 * On a plank or a pull-up the number is load you ADDED to your own body, not the load
 * itself — and «Вага, кг» sitting above a 0 reads as "this exercise weighs nothing"
 * rather than "no extra plates today".
 */
export function weightFieldLabel({ timed = false, bodyweight = false } = {}) {
    return timed || bodyweight ? "Додаткова, кг" : "Вага, кг";
}

/** True when the exercise is done against your own body rather than an external load. */
export function isBodyweightExercise(exercise) {
    return Boolean(exercise) && exercise.equipment === "Вага тіла";
}
