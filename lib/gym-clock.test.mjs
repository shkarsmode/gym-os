...
test("the forgotten session: an 18-hour-old active workout never presents as live", () => {
    ...
    // lunch today. This is the case the whole cap exists for — the chip must read the start,
    ...
    // And the label must SAY it was yesterday. A bare "18:05" reads as this evening —
    // wrong in precisely the scenario this branch exists to describe.
    assert.equal(state.startLabel, "вчора 18:05");
});

test("startedAtLabel names the day once the session did not start today", () => {
    const noon = new Date(2026, 7, 20, 12, 0);
    assert.equal(startedAtLabel(new Date(2026, 7, 20, 18, 5), noon), "18:05", "today needs no date");
    assert.equal(startedAtLabel(new Date(2026, 7, 19, 18, 5), noon), "вчора 18:05");
    assert.match(startedAtLabel(new Date(2026, 7, 14, 7, 30), noon), /^14 .* 07:30$/);
    assert.equal(startedAtLabel("не дата", noon), "");
});
...
// The exact regression seen on production: tick one set, then press «Завершити» while
// four sets are still un-ticked. The sweep marks them done, so the session's end is now —
// not the single early tick, which froze the chip at 0:00 and suppressed the suggestion.
test("finishing with un-ticked sets ends the clock at the sweep, not at the one early tick", () => {
    const start = Date.parse("2026-08-20T18:00:00.000Z");
    const sweptAt = start + 5 * 60 * 1000;
    const finished = {
        status: "completed",
        firstSetAt: new Date(start).toISOString(),
        lastSetAt: new Date(sweptAt).toISOString(),
        exercises: [{ sets: [{ isCompleted: true }, { isCompleted: true }, { isCompleted: true }] }]
    };
    const clock = gymClockState(finished, sweptAt + 60 * 1000);
    assert.equal(clock.ms, 5 * 60 * 1000);
    assert.equal(clock.live, false);
    assert.equal(suggestedDurationMinutes(clock.ms), 15, "a real span must still produce an offer");
});

// The full cycle an athlete can actually perform: tick, change your mind and clear every
// box, then start over. The clock must forget the first attempt completely — carrying the
// old anchor forward would show a session that "started" before the work did.
test("clearing every checkbox resets the clock, and re-ticking starts a fresh one", () => {
    const item = {
        status: "active",
        firstSetAt: null,
        lastSetAt: null,
        exercises: [{ sets: [{ isCompleted: false }, { isCompleted: false }] }]
    };
    const firstTry = new Date("2026-08-20T10:00:00.000Z");
    item.exercises[0].sets[0].isCompleted = true;
    Object.assign(item, nextGymClockMarks(item, firstTry));
    assert.equal(item.firstSetAt, firstTry.toISOString());

    item.exercises[0].sets[0].isCompleted = false;
    Object.assign(item, nextGymClockMarks(item, new Date("2026-08-20T10:05:00.000Z")));
    assert.equal(item.firstSetAt, null, "no completed sets left means no session in progress");
    assert.equal(item.lastSetAt, null);
    assert.equal(gymClockState(item, Date.now()), null, "and no chip at all");

    const restart = new Date("2026-08-20T10:30:00.000Z");
    item.exercises[0].sets[1].isCompleted = true;
    Object.assign(item, nextGymClockMarks(item, restart));
    assert.equal(item.firstSetAt, restart.toISOString(), "the clock restarted from the new first tick");
    assert.equal(gymClockState(item, restart.getTime() + 60_000).ms, 60_000);
});
