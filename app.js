    ...
    function setRow(workoutExerciseId, set, readonly, index) {
        ...
        return `<div class="set-row ${set.isCompleted ? "completed" : ""}">
            <div class="set-row-head">
                ...
                <select class="set-type-select" data-action="set-field" ${target} data-field="type" ${lock}>${["warmup", "working", "drop", "failure", "backoff"].map((type) => `<option value="${type}" ${set.type === type ? "selected" : ""}>${setTypeLabel(type)}</option>`).join("")}</select>
