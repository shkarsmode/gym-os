(() => {
    ...
    function achievementsPage() {
        ...
        content(`<section class="card"><div class="card-header"><div><h2>Досягнення</h2><p class="card-caption">Стримані бейджі зі станами locked/unlocked, прогресом і датою відкриття.</p></div><span class="badge accent">${items.filter((item) => item.isUnlocked).length}/${items.length} відкрито</span></div><div class="tab-row">${categories.map((category) => `<button class="segment-button ${state.filters.achievementCategory === category ? "active" : ""}" type="button" data-action="achievement-filter" data-category="${category}">${category === "all" ? "Усі" : category}</button>`).join("")}</div></section><section class="achievement-grid" style="margin-top:16px;">${filtered.map(achievementCard).join("")}</section>`);
    }
